...
public class UserService extends Services implements UserInterface {
	...
	public User logIn(String userName, String passWord)
		...
		try (Connection conn = abrirConexion()) {
			...
			if (u1 == null) {
				...
				if(u1.getPassword().equals(DigestUtils.sha256Hex(passWord))) {
					u1.setLogInDate(LocalDate.now());
				}else {
	...
	@Override
	public User checkUser(Integer id) throws UserServiceException, UserDoesntExistException {
		try (Connection conn1 = abrirConexion()) {
			User u1 = userDao.getById(conn1,id);
			if (u1 == null) {
